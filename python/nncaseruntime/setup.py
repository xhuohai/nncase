...
class BuildCMakeExt(build_ext):
    ...
    def build_cmake(self, ext: Extension):
        ...
        self.spawn(["conan", "install", ext.sourcedir, "--build=missing", "-s",
            ...
            "-o", "&:runtime=True", "-o", "&:python=True", "-o", "&:tests=False", "-o", f"&:python_root={os.path.dirname(sys.executable)}",
            "-c", f"tools.cmake.cmake_layout:build_folder={self.build_temp}", "-o" ,f"&:Python3_INCLUDE_DIRS={os.path.join(os.path.dirname(sys.executable), 'include/python3.10')}"])
        ...
        nncase_libs = [os.path.join(root, _lib) for root, _, files in
                ...
                os.path.isfile(os.path.join(root, _lib)) and
                (os.path.splitext(_lib)[-1] in [".so", ".json"] or
                _lib.startswith("lib"))]
